...
describe("freezer demand for a fortnight", () => {
  it("keeps a fortnight inside three freezing waves", () => {
    ...
    for (let seed = 0; seed < 20; seed++) {
      const plan = generatePlan({ settings: { coverage: weeks(2) }, restarts: 40, seed });
  ...
  it("needs fewer waves when there are more trays", () => {
    const few = generatePlan({
      settings: { coverage: weeks(2), freezer: { ...DEFAULT_FREEZER, trayCount: 3 } },
      restarts: 20, seed: 1,
    });
    const many = generatePlan({
      settings: { coverage: weeks(2), freezer: { ...DEFAULT_FREEZER, trayCount: 12 } },
      restarts: 20, seed: 1,
    });
  ...
  it("uses the fresh and no-cook pool when the freezer is under pressure", () => {
    ...
    const plan = generatePlan({
      settings: { coverage: weeks(2), slots: ["breakfast", "lunch"] },
      restarts: 40, seed: 3,
    });
  ...
  it("needs fewer waves for lunches alone than for both meals", () => {
    const lunchOnly = generatePlan({ settings: { coverage: weeks(2), slots: ["lunch"] }, restarts: 20, seed: 2 });
    const both = generatePlan({
      settings: { coverage: weeks(2), slots: ["breakfast", "lunch"] }, restarts: 20, seed: 2,
    });
...
describe("the plan is actually batch-cooked", () => {
  it("does not let cold-assembly meals take over", () => {
    ...
    for (let seed = 0; seed < 15; seed++) {
      const plan = generatePlan({
        settings: { coverage: weeks(2), slots: ["breakfast", "lunch"] }, restarts: 30, seed,
      });
