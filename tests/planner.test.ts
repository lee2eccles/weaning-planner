...
describe("H1 — no plan ever contains a legume", () => {
  it.each(WEEKS)("holds for a %i-week plan across many seeds", (w) => {
    for (let seed = 0; seed < 25; seed++) {
      const plan = generatePlan({ settings: { meals: w * 7 }, restarts: 5, seed });
...
describe("H3/H4 — variety", () => {
  it.each(WEEKS)("never repeats a meal on consecutive days (%i weeks)", (w) => {
    const plan = generatePlan({ settings: { meals: w * 7 }, restarts: 20, seed: 42 });
    for (const meal of plan.meals) {
  ...
  it.each(WEEKS)("serves nothing more than twice in one week (%i weeks)", (w) => {
    const plan = generatePlan({ settings: { meals: w * 7 }, restarts: 20, seed: 7 });
...
describe("H5/H6 — fridge life and freezing", () => {
  it("never serves a fridge-only recipe beyond its keeping time", () => {
    const plan = generatePlan({ settings: { meals: 28 }, restarts: 20, seed: 3 });
    for (const meal of plan.meals) {
      const r = getRecipe(meal.recipeId);
  ...
  it("assigns a defrost cube count to every frozen cube meal", () => {
    const plan = generatePlan({ settings: { meals: 14 }, restarts: 10, seed: 11 });
...
describe("H8 — prep cadence", () => {
  it("uses one prep session for up to 2 weeks and two beyond", () => {
    expect(generatePlan({ settings: { meals: 7 }, restarts: 5, seed: 1 }).prepSessions).toHaveLength(1);
    expect(generatePlan({ settings: { meals: 14 }, restarts: 5, seed: 1 }).prepSessions).toHaveLength(1);
    expect(generatePlan({ settings: { meals: 21 }, restarts: 5, seed: 1 }).prepSessions).toHaveLength(2);
    expect(generatePlan({ settings: { meals: 28 }, restarts: 5, seed: 1 }).prepSessions).toHaveLength(2);
  });
});

describe("H9 — tray throughput", () => {
  it("never puts more trays in a wave than exist", () => {
    const plan = generatePlan({ settings: { meals: 28 }, restarts: 20, seed: 5 });
  ...
  it("never puts more cubes in a tray than it holds", () => {
    const plan = generatePlan({ settings: { meals: 28 }, restarts: 20, seed: 6 });
    for (const s of plan.prepSessions) {
      for (const w of s.waves) {
  ...
  it("keeps one recipe per tray, so every bag is a single recipe", () => {
    const plan = generatePlan({ settings: { meals: 14 }, restarts: 20, seed: 8 });
    for (const s of plan.prepSessions) {
      for (const w of s.waves) {
        for (const t of w.trays) {
...
describe("portion and cube maths", () => {
  ...
  it("produces enough portions for every meal it is scheduled for", () => {
    const plan = generatePlan({ settings: { meals: 14 }, restarts: 20, seed: 13 });
    for (const s of plan.prepSessions) {
...
describe("optimisation goals", () => {
  it("fills every meal slot", () => {
    for (const w of WEEKS) {
      for (const slots of [["lunch"], ["breakfast", "lunch"]] as MealSlot[][]) {
        // Meals, not days: a week of two slots is fourteen meals.
        const meals = w * 7 * slots.length;
        const plan = generatePlan({ settings: { meals, slots }, restarts: 20, seed: 21 });
        expect(plan.meals, `${w}w ${slots.join("+")}`).toHaveLength(meals);
      }
    }
  });

  it("plans lunches only when that is all that is switched on", () => {
    const plan = generatePlan({ settings: { meals: 14, slots: ["lunch"] }, restarts: 10, seed: 1 });
  ...
  it("reuses ingredients — at least 35% fewer distinct than the naive sum", () => {
    const plan = generatePlan({ settings: { meals: 14 }, restarts: 50, seed: 2 });
  ...
  it("generates a 4-week plan in under a second", () => {
    ...
    generatePlan({ settings: { meals: 28 }, restarts: 200, seed: 99 });
    expect(Date.now() - start).toBeLessThan(1000);
  });

  it("is deterministic for a given seed", () => {
    const a = generatePlan({ settings: { meals: 14 }, restarts: 10, seed: 123 });
    const b = generatePlan({ settings: { meals: 14 }, restarts: 10, seed: 123 });
...
describe("manual override", () => {
  it("offers only legal swaps", () => {
    const plan = generatePlan({ settings: { meals: 14 }, restarts: 10, seed: 31 });
  ...
  it("keeps locked meals exactly where they were", () => {
    const plan = generatePlan({ settings: { meals: 14 }, restarts: 10, seed: 41 });
    const locked = [{ ...plan.meals[4], locked: true }];
    const regenerated = generatePlan({ settings: { meals: 14 }, restarts: 10, seed: 77, locked });
...
describe("batch cooking vs cooking on the day", () => {
  it("never batch-cooks a meal to be eaten beyond its fridge life", () => {
    ...
    for (let seed = 0; seed < 15; seed++) {
      const plan = generatePlan({ settings: { meals: 14 }, restarts: 10, seed });
  ...
  it("lists cook-fresh meals separately rather than batching them", () => {
    const plan = generatePlan({ settings: { meals: 14 }, restarts: 40, seed: 3 });
    for (const session of plan.prepSessions) {
